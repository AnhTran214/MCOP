...
import {FlatList, StyleSheet, Text, View, Image, Alert, Platform, TouchableHighlight, 
        RefreshControl, TextInput,ScrollView,Modal} from 'react-native';
...
setPickerValue(newValue) {
  ...
  getUserFromDB() {
    return new Promise((resolve) => {
        LearnAppUser.orderByChild('email')
  ...
  async update() {
        try {
            await LearnAppUser.orderByChild('id')
                .equalTo(this.state.userData.id)
                .on('child_added', (data) => {
                    data.key;
                    LearnAppUser.child(data.key).update({
        ...
        return(
            <View style = {{ flex: 1, marginTop: Platform.OS === 'ios' ? 34:0 }}>
            	...
            	<Text style = {{
                      ...
                      <View style={{ alignSelf: 'flex-start', marginTop: '1%', marginLeft: '2%',flexDirection:'column'}}>
                      <Text style = {{
                              marginLeft: '2%',
                      ...
                      <View style={[styles.propertyValueRowView]}>
					    ...
					    underlineColorAndroid="transparent"
					    placeholderTextColor = 'grey'
					    placeholder={this.state.itemData.password}
                        autoCapitalize='none'
                        editable = {false}
				        />
				        </View>
